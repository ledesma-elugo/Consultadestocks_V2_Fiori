sap.ui.define([
	...
	return Controller.extend("ar.ledesma.fiori.mm.consultastock.controller.Principal", {

		/**
		 * Called when a controller is instantiated and its View controls (if available) are already created.
		 * Can be used to modify the View before it is displayed, to bind event handlers and do other one-time initialization.
		 * @memberOf ar.ledesma.fiori.mm.consultastock.view.Principal
		 */
		oDataTabla: {
			Stocks: "",
			Reservas: "",
		...
		onInit: function () {
			...
			this._getPlants();
			this.getOwnerComponent().getModel("mGeneral").setProperty("/plantSelectedKey", "");
			this.getOwnerComponent().getModel("mGeneral").setProperty("/warehouseSelectedKey", "");
		},

		gatewayRead: function(oDataModel, sEntity, mOptions){
			var oContext = this;
			return new Promise(function(fnSuccess, fnError){
				...
				oDataModel.read(sEntity, mOptions);
				//sModelName
			});
		},
		...
		onLgortSelected: function (controlEvento) {
			// revisar
			// en este se usa "searchFieldPlant" para obtener el centro seleccionado
			// var ServiceURL = "/sap/opu/odata/sap/ZMM_CONSULTA_STOCK_SRV";
			
			// var oModelRead = new sap.ui.model.odata.v2.ODataModel(ServiceURL);
			var oModelRead = this.getOwnerComponent().getModel("CONSULTA_STOCK");
			// var oModelNecesidadesRead = new sap.ui.model.odata.v2.ODataModel(ServiceURL);
			var oModelNecesidadesRead = this.getOwnerComponent().getModel("CONSULTA_STOCK");
			// var oModelMovMercanciasRead = new sap.ui.model.odata.v2.ODataModel(ServiceURL);
			var oModelMovMercanciasRead = this.getOwnerComponent().getModel("CONSULTA_STOCK");
			// var oModelStockRead = new sap.ui.model.odata.v2.ODataModel(ServiceURL);
			var oModelStockRead = this.getOwnerComponent().getModel("CONSULTA_STOCK");
			...
			var oBuscadorCentro = this.getView().byId("comboBoxPlant");
			...
			var Werks = new sap.ui.model.Filter({
				...
				value1: oBuscadorCentro.getSelectedKey()
			});
			...
			aFilterStock.push(new sap.ui.model.Filter("Plant", "EQ", oBuscadorCentro.getSelectedKey()));
		...
		onLgortSelectedM: function (controlEvento) {
			...
			var oBuscadorCentro = this.getView().byId("comboBoxPlantM");
		...
		onPress: function (oEvent) {
			// revisar
			// acá se usa "searchFieldPlant" para obtener el centro
			// var oBuscadorAlmacen = this.getView().byId("searchFieldWarehouse");
			var oBuscadorAlmacen = this.getView().byId("comboBoxWarehouse");
			// var oBuscadorCentro = this.getView().byId("searchFieldPlant");
			var oBuscadorCentro = this.getView().byId("comboBoxPlant");
			var oBuscadorMaterial = this.getView().byId("Material");
			...
			if (!oBuscadorMaterial.getValue() || !oBuscadorCentro.getSelectedKey()) {
				...
				oModelRead.attachRequestCompleted(function () {
					var nlength = oDataRead.results.length;
		...
		onPressM: function (oEvent) {
			...
			var oBuscadorAlmacen = this.getView().byId("comboBoxWarehouseM");
			// var oBuscadorCentro = this.getView().byId("searchFieldPlantM");
			var oBuscadorCentro = this.getView().byId("comboBoxPlantM");
			var oBuscadorMaterial = this.getView().byId("MaterialM");
			var oVista = this.getView();
			...
			if (!oBuscadorMaterial.getValue() || !oBuscadorCentro.getSelectedKey()) {// revisar
				tabla.setVisible(true);
			} else {
				tabla.setVisible(true);
				// var ServiceURL = "/sap/opu/odata/sap/ZMM_CONSULTA_STOCK_SRV";
		...
		_getWarehouses: function (){
			var oModelRead = this.getOwnerComponent().getModel("CONSULTA_STOCK");
			var entity = "/WarehouseSet";
			var oContext = this;
			
			var oBuscadorCentro = this.getView().byId("comboBoxPlant");
			var Centro = oBuscadorCentro.getSelectedKey();
			...
			function fnSuccess(oDataR, oResponse) {
				if(oDataR !== undefined && oDataR !== ""){
					var aWarehouses = oDataR.results;
					oContext.getOwnerComponent().getModel("mGeneral").setProperty("/warehouses", aWarehouses);
				}else{
					oContext.getOwnerComponent().getModel("mGeneral").setProperty("/warehouses", []);
				}
			}
		...
		_getPlants: function () {
			...
			function fnSuccess(oDataR, oResponse) {
				if(oDataR !== undefined && oDataR !== ""){
					var aPlants = oDataR.results;
					oContext.getOwnerComponent().getModel("mGeneral").setProperty("/plants", aPlants);
				}else{
					oContext.getOwnerComponent().getModel("mGeneral").setProperty("/plants", []);
				}
			}
		...
		/*onSuggestPlant: function (oEvent) {
			// revisar
			// acá se usa "searchFieldPlant" para obtener las sugenerencias.
			var oBuscador = this.getView().byId("searchFieldPlant");
			// var ServiceURL = "/sap/opu/odata/sap/ZMM_CONSULTA_STOCK_SRV";
			// var oModelRead = new sap.ui.model.odata.v2.ODataModel(ServiceURL);
			var oModelRead = this.getOwnerComponent().getModel("CONSULTA_STOCK");
			...
			// });
				...
				debugger;
				this.gatewayRead(oModelRead, entity, {"filters": oFilters})
				.then(function(oDataR, oResponse){
					console.log("logSuccess");
					var oDataResults = oDataR;
					console.log("logSuccess");
					oBuscador.removeAllSuggestionItems();
					if(oDataResults !== undefined){
						var nlength = oDataResults.results.length;
						for (var j = 0; j < nlength; j++) {
							var sugerencia = new sap.m.SuggestionItem();
							sugerencia.setDescription(oDataResults.results[j].PlantP + " - " + oDataResults.results[j].Description);
							sugerencia.setKey(oDataResults.results[j].PlantP);
							oBuscador.addSuggestionItem(sugerencia);
						}
					}
		...
		onHandleValueHelp: function (oEvent) {
			...
			if (!this._valueHelpDialog) {
				this._valueHelpDialog = sap.ui.xmlfragment("ar.ledesma.fiori.mm.consultastock.view.AyudaBusquedaMaterial", this);
		...
		_handleValueHelpSearch: function (evt) {
			// revisar , ayuda de busqueda para el material
			// se usa searchFieldPlant" para obtener el centro
